...
public class HLSion {
    ...
    public var localUrl: URL? {
        ...
        return DownloadSessionManager.shared.homeDirectoryURL.appendingPathComponent(relativePath)
    }
    /// Download state.
    public var state: State {
        if DownloadSessionManager.shared.assetExists(forName: name) {
            return .downloaded
        }
        if let _ = DownloadSessionManager.shared.downloadingMap.first(where: { $1 == self }) {
            return .downloading
        }
    ...
    public var offlineAssetSize: UInt64 {
        ...
        let bundleURL = DownloadSessionManager.shared.homeDirectoryURL.appendingPathComponent(relativePath)
...
//    internal var resolvedMediaSelection: AVMediaSelection?
    ...
    public static func restoreDownloadsTasks() {
        _ = DownloadSessionManager.shared
    }
    ...
    public func download(progress closure: ProgressParameter? = nil) -> Self {
        ...
        DownloadSessionManager.shared.downloadStream(self)
        return self
    }
    ...
    public func cancelDownload() {
        DownloadSessionManager.shared.cancelDownload(self)
    }
    ...
    public func deleteAsset() throws {
        try DownloadSessionManager.shared.deleteAsset(forName: name)
    }
